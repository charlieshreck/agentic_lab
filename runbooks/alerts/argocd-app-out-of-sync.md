...
| **Severity** | Warning |
| **Threshold** | App not Synced for 30 minutes (health must be Degraded/Unknown/Missing — Healthy/Progressing excluded) |
...
### 6. Transient OutOfSync After Git Push (NOT an incident — FP)

**Symptoms:**
- App is `OutOfSync` but `health: Healthy`
- Recent Git commit visible in repo history for this app's path
- ArgoCD auto-sync is enabled for the app
- Resolves on its own within 2–5 minutes (ArgoCD reconcile period)

**Root cause:**
When a manifest is committed and pushed to Git, ArgoCD detects the diff immediately via webhook/polling. The live cluster doesn't change until ArgoCD's auto-sync runs. This creates a brief window where sync_status=OutOfSync while health=Healthy (the existing deployment is still running fine).

**This is normal and expected.** The alert rule now excludes `Healthy|Progressing` apps:
```
argocd_app_info{sync_status!="Synced", health_status!~"Healthy|Progressing"} == 1 for: 30m
```

**Action required:** None. If KAO creates a finding for this, classify as **false positive (fp)**. ArgoCD auto-sync will resolve it.

**If not resolved after 10+ minutes:**
- Check if auto-sync is actually enabled: look for `autosync_enabled: true` in ArgoCD metrics
- Manually trigger sync: `mcp__infrastructure__argocd_sync_application(name="<app-name>")`

**Pattern first seen:** Incident #1393 — gatus ConfigMap change (2026-03-26)
**Alert rule fix commit:** `f31b33e` (monit_homelab) — excludes Healthy/Progressing apps

---

### 7. Target Cluster Unreachable
...
### Incident #224 / Finding #1399 — Transient OutOfSync on gatus (2026-03-26)

**Alert**: ArgoCDAppOutOfSync on gatus (health: Healthy)
**Status**: RESOLVED (auto-resolved by ArgoCD sync)
**Root Cause**: Commit `ae3105e` changed gatus ConfigMap (Backrest URL from internal DNS `backrest-external.apps.svc:9898` to Monit Traefik LB IP `10.10.0.31`). Gatus entered OutOfSync+Healthy state for ~5 minutes before ArgoCD auto-synced it.

**Why it alerted (when it shouldn't):** The Prometheus rule at the time was `argocd_app_info{sync_status!="Synced"} == 1 for: 30m`, which would catch Healthy apps too. The rule was updated in commit `f31b33e` at 05:19 UTC (13 min after the gatus change) to exclude `Healthy|Progressing` apps. The KAO finding was generated by direct ArgoCD state polling before the new rule propagated.

**Fix applied:** Alert rule updated in `f31b33e` to `health_status!~"Healthy|Progressing"` exclusion.

**Pattern:** Any ConfigMap, Deployment, or other manifest change will cause transient OutOfSync. With auto-sync enabled, this always self-resolves. See Cause 6 above for the suppression pattern.
