...
### February 2026 - claude-validator-daily Failed (Orphaned App)
- **Job:** claude-validator-daily-29529000
- **Cause:** Application was retired (moved to archive). The ArgoCD Application resource was no longer in git, but the CronJob and Service lingered on the cluster. The CronJob tried to call a non-existent deployment.
- **Resolution:**
  1. Restored claude-validator manifests from archive to active git (kubernetes/applications/claude-validator/)
  2. Added `suspend: true` to the CronJob spec to prevent future failures
  3. Placed the ArgoCD Application in kubernetes/argocd-apps/ for discovery
  4. Committed and pushed - ArgoCD will now manage the suspended CronJob
- **Lesson:** Orphaned resources linger on the cluster even after archiving. If archiving an application, either delete the Application resource first (triggering cleanup) or restore it to git with a suspended/disabled state until cleanup is complete.
